...
#tan_ab_dict: (ec,station,ring):(a,b)--a is y int, b slope of tan(theta) (y) vs. wire number (x)
tan_ab_dict = {(1,1,3):(-0.740587,-0.00737487),(1,1,2):(-17.6517,0.301504),(1,2,2):(-0.468672,-0.00565595),(1,3,2):(-0.446212,-0.00449382),(1,4,2):(-0.451912,-0.00352335),(2,1,3):(0.737727,0.0075506),(2,1,2):(0.602858,0.00102775),(2,2,2):(0.460577,0.00586943),(2,3,2):(0.437834,0.00469009),(2,4,2):(0.538971,0.00215101)}
...
tank_222_213 = ROOT.TH2D("tank_222_213","($\Delta Tan(\Theta)$) vs. Curvature; $\Kappa$; ($\Delta Tan(\Theta)$)",100,-0.3,0.3,100,-0.3,0.3)
tank_242_232 = ROOT.TH2D("tank_242_232","($\Delta Tan(\Theta)$) vs. Curvature; $\Kappa$; ($\Delta Tan(\Theta)$)",100,-0.3,0.3,100,-0.3,0.3)
tank_232_222 = ROOT.TH2D("tank_232_222","($\Delta Tan(\Theta)$) vs. Curvature; $\Kappa$; ($\Delta Tan(\Theta)$)",100,-0.3,0.3,100,-0.3,0.3)
tank_142_132 = ROOT.TH2D("tank_142_132","($\Delta Tan(\Theta)$) vs. Curvature; $\Kappa$; ($\Delta Tan(\Theta)$)",100,-0.3,0.3,100,-0.3,0.3)
tank_132_122 = ROOT.TH2D("tank_132_122","($\Delta Tan(\Theta)$) vs. Curvature; $\Kappa$; ($\Delta Tan(\Theta)$)",100,-0.3,0.3,100,-0.3,0.3)
tank_122_113 = ROOT.TH2D("tank_122_113","($\Delta Tan(\Theta)$) vs. Curvature; $\Kappa$; ($\Delta Tan(\Theta)$)",100,-0.3,0.3,100,-0.3,0.3)
	    ...
	    for s in cscMatchedSegments:
			...
			if ((station2,ring2),(station1,ring1)) in [((4,2),(3,2)),((3,2),(2,2)),((2,2),(1,3))]:
			    k = float(g.charge()/g.pt())
		    	    a2, b2 = tan_ab_dict[(ec2,station2,ring2)]
			    a1, b1 = tan_ab_dict[(ec1,station1,ring1)]
				...
				tank_222_213.Fill(k,delta_tan_theta)
