...
#in process of cleaning up: ### = probably can delete, will need to run code with these lines commented before deletion just to verify
    ...
    def stubThresholdTest(self,stub):
        ...
        if (ec,station,ring) in ec_s_r_dict:
            ...
            if abs(phi_diff/phi_res) < phi_threshold and abs(wire_diff/wire_res) < wire_threshold:
                return stub 
            #else:
                #print 'there was a hit here: {}, but it didnt pass threshold'.format((ec,station,ring))
                #print 'the difference in phi/resolution was {} and the threshold is {}'.format(abs(phi_diff/phi_res),phi_threshold)
            #    print 'gen phi: ',self.gen_phi
            #    print 'my calcualted phi: ',self.getPhi(stub)
            #    print 'propagated phi: ',self.phiPropagated((ec,station,ring))
            #    print 'difference in phi: ',phi_diff
            #    print 'resolution: ',phi_res
            #    
            #    #print 'the differnece in wire/res was ',abs(wire_diff/wire_res) + 'and the threshold is ',wire_threshold
